...
import {
  RiWeightLine, RiLeafLine, RiLineChartLine, RiCapsuleLine,
  RiYoutubeLine, RiCalendarCheckLine, RiArrowRightLine,
  RiFireLine, RiTrophyLine, RiShieldCheckLine, RiFlashlightLine,
  RiHeartPulseLine, RiBodyScanLine, RiTimeLine, RiCheckLine,
} from 'react-icons/ri';
import './Landing.css';
...
const FEATURES = [
  { icon: RiWeightLine,       color: '#a0ff6f', label: 'Workout Tracking',    desc: 'Log sets, reps, and rest. Build a full 6-day split.' },
  { icon: RiLeafLine,         color: '#34d399', label: 'Diet & Macros',        desc: 'Auto-fill nutrition from 50+ foods. Track every meal.' },
  { icon: RiLineChartLine,    color: '#60a5fa', label: 'Analytics',            desc: 'Visualise progress over time with beautiful charts.' },
  { icon: RiCapsuleLine,      color: '#f59e0b', label: 'Supplements',          desc: 'Log your full supplement stack with dosage & timing.' },
  { icon: RiCalendarCheckLine,color: '#a78bfa', label: 'Gym Plans',            desc: 'Start from a Beginner, Intermediate, or Advanced preset.' },
  { icon: RiYoutubeLine,      color: '#f87171', label: 'Video Suggestions',    desc: 'AI-curated workout videos matched to your training split.' },
];

const PLAN_BADGES = [
  { label: 'Full Body Split', sub: '3 days / week', badge: 'Beginner', color: 'var(--info)' },
  { label: 'Push/Pull/Legs',  sub: '4 days / week', badge: 'Intermediate', color: 'var(--accent-dark)' },
  { label: 'PPL — 6 Days',    sub: '6 days / week', badge: 'Advanced', color: 'var(--danger)' },
];
    ...
    <div className="landing">
      ...
      <header className="landing-nav">
        ...
        <div className="landing-nav-right">
          <ThemeToggle />
          <Link to="/login" className="btn btn-ghost landing-nav-btn">Log in</Link>
      ...
      <section className="landing-hero">
        ...
        <p className="landing-hero-sub">
          Workouts, diet, supplements, analytics — all in one brutally simple app.
        </p>
          ...
          {STATS.map(s => (
            <div key={s.label} className="landing-stat">
              <div className="landing-stat-val">
                <AnimatedCounter target={s.value} /><span className="landing-stat-unit"> {s.unit}</span>
              </div>
      ...
      <section className="landing-bento-section">
        <div className="landing-bento">
          ...
          <div className="bento-tile bento-tile--xl bento-workout">
            ...
            <h2 className="bento-tile-title">Build your<br/>perfect split</h2>
            <p className="bento-tile-desc">6-day planner. Log exercises, sets, reps, and rest. Load a preset or build from scratch.</p>
          ...
          <div className="bento-tile bento-diet">
            ...
            <p className="bento-tile-desc-sm">Type a food, get macros instantly from a 50+ item database.</p>
          ...
          <div className="bento-tile bento-analytics">
            ...
            <div className="bento-chart-mock">
              {[40, 55, 48, 70, 63, 80, 75].map((h, i) => (
                <div key={i} className="bento-bar" style={{ height: `${h}%` }} />
              ))}
            </div>
          </div>
            ...
            <div className="bento-plan-list">
              {PLAN_BADGES.map(p => (
                <div key={p.label} className="bento-plan-item">
                  <span className="bento-plan-badge" style={{ color: p.color, borderColor: p.color }}>{p.badge}</span>
          ...
          <div className="bento-tile bento-supps">
            ...
            <p className="bento-tile-desc-sm">Creatine, whey, vitamins — log dosage and timing every day.</p>
            <div className="bento-supp-icons">
              {['💊', '🧪', '⚡', '🫀'].map(e => (
                <span key={e} className="bento-supp-icon">{e}</span>
              ))}
            </div>
          </div>
          ...
          <div className="bento-tile bento-videos">
            ...
            <p className="bento-tile-desc-sm">AI-matched video tutorials based on your current workout plan.</p>
          ...
          <div className="bento-tile bento-metrics">
            <div className="bento-metrics-content">
              <div className="bento-tile-label"><RiHeartPulseLine size={14} /> Health Metrics</div>
              <h3 className="bento-tile-title-sm">BMI · Streak · Calories · Macros</h3>
              <p className="bento-tile-desc-sm">Your complete fitness dashboard at a glance.</p>
            </div>
            <div className="bento-metrics-tags">
              {['BMI Calculator', 'Calorie Goal', 'Macro Rings', 'Weekly Streak', 'Body Scan', 'Export Data'].map(t => (
        ...
        <div className="landing-features-grid">
          {FEATURES.map(f => (
            <div key={f.label} className="landing-feature-card">
              <div className="landing-feature-icon" style={{ background: f.color + '22', border: `2px solid ${f.color}` }}>
      ...
      <section className="landing-cta-section">
        <div className="landing-cta-card">
          <div className="landing-cta-icon"><RiTrophyLine size={32} /></div>
          <h2 className="landing-cta-title">READY TO START YOUR<br/>FITNESS JOURNEY?</h2>
          <p className="landing-cta-sub">Free forever. No credit card. Just gains.</p>
      ...
      <footer className="landing-footer">
        <div className="landing-footer-logo">dev<span>Fit</span></div>
        <div className="landing-footer-links">
          <Link to="/login">Login</Link>
          <Link to="/register">Register</Link>
        </div>
        <div className="landing-footer-copy">Built for athletes. Powered by data.</div>
      </footer>
    </div>
  );
};
