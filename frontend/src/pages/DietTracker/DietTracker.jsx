...
const lookupNutrition = (foodName, quantity = 100) => {
  ...
  const entry = Object.entries(FOOD_DB).find(
...
const estimateMicros = (entries = []) => {
  ...
  entries.forEach((e) => {
    ...
    const match = Object.entries(FOOD_DB).find(
      ([k]) => key === k || key.includes(k) || k.includes(key),
    );
    if (!match) return;
...
const DietTracker = () => {
  ...
  const fetchDiet = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/diet?date=${filterDate}`);
      setDiet(data.diet);
    } catch {
      setError("Failed to load diet log.");
    } finally {
      setLoading(false);
    }
  }, [filterDate]);
  ...
  const handleAdd = async (e) => {
    ...
    setError("");
    try {
      const { data } = await api.post("/diet/entry", {
        ...form,
        date: filterDate,
  ...
  const handleDelete = async (entryId) => {
    try {
      const { data } = await api.delete(
        `/diet/entry/${entryId}?date=${filterDate}`,
      );
      setDiet(data.diet);
    } catch {
  ...
  const grouped = MEAL_TIMES.reduce((acc, mt) => {
    ...
    if (entries.length > 0) acc[mt] = entries;
    return acc;
  }, {});
      ...
      <main className="page-content">
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle">Log meals · auto-analyse macros.</p>
          </div>
        </div>
        ...
        <div className="card diet-summary">
          <div className="diet-cal-block">
            ...
            <div className="diet-cal-bar-wrap">
              <div
                className="diet-cal-bar"
                style={{
                  width: `${Math.min(100, (totals.totalCalories / calorieGoal) * 100)}%`,
                }}
              />
            </div>
            ...
            <form onSubmit={handleAdd}>
              <div className="diet-top-fields">
                ...
                >
                  ...
                  <div className="diet-food-wrap">
                    ...
                    <input
                      className="diet-food-input"
                      value={form.foodName}
                      onChange={update("foodName")}
                      placeholder="e.g. Chicken Breast, Rice, Egg…"
                      required
                      autoComplete="off"
                    />
                    {autoFilled && (
                      <span className="diet-auto-badge">
                        <RiCheckLine size={10} /> Auto
                      </span>
                    )}
                  </div>
                    ...
                    <div className="diet-suggestions">
                      {suggestions.map((s) => (
                        <button
                          key={s}
                          type="button"
                          className="diet-suggestion-item"
              ...
              <div className="diet-macros-grid">
                {[
                  ...
                  <div
                    key={f}
                    className={`diet-macro-field ${autoFilled ? "diet-macro-field--auto" : ""}`}
                  >
              ...
              <div className="diet-form-actions">
                ...
                >
                  {saving ? (
                    <>
                      <RiRefreshLine size={14} className="spin" /> Adding…
                    </>
                  ) : (
                    <>
                      <RiCheckLine size={14} /> Add Food
                    </>
                  )}
                </button>
              </div>
        ...
        <div className="diet-meals">
          {loading ? (
            <div className="dash-loading">Loading diet log…</div>
          ) : Object.keys(grouped).length === 0 ? (
            <div className="empty-state">
              <RiLeafLine size={28} className="empty-state-icon" />
              <div className="empty-state-text">
                No meals logged. Add food above.
              </div>
            </div>
          ) : (
              ...
              return (
                <div key={mealTime} className="diet-meal-group">
                  <div className="diet-meal-header">
                    <span className="diet-meal-emoji">
                      {MEAL_ICONS[mealTime]}
                    </span>
                    <span className="diet-meal-name">
                      {mealTime.charAt(0).toUpperCase() + mealTime.slice(1)}
                    </span>
                    <span className="diet-meal-cals">
                      <RiFireLine size={12} />
                      {mealCals} kcal
                    </span>
                  </div>
                  {entries.map((entry) => (
                    <div key={entry._id} className="diet-entry">
                      <div className="diet-entry-left">
                        ...
                        <div className="diet-entry-qty">
                          {entry.quantity}
                          {entry.unit}
                        </div>
                      </div>
                      <div className="diet-entry-macros">
                        <span className="diet-mpill diet-mpill-cal">
                          <RiFireLine size={10} />
                          {entry.calories}
                        </span>
                        <span className="diet-mpill diet-mpill-p">
                          P {entry.protein}g
                        </span>
                        <span className="diet-mpill diet-mpill-c">
                          C {entry.carbs}g
                        </span>
                        <span className="diet-mpill diet-mpill-f">
                          F {entry.fats}g
                        </span>
                      </div>
                      <button
                        className="btn btn-danger diet-del-btn"
          ...
          {showMicros && (
            <>
                ...
                {[
                  ...
                  <div key={m.label} className="diet-micro-item">
                    <div className="diet-micro-header">
                      ...
                      <span
                        className="diet-micro-val"
                        style={{ color: m.color }}
                      >
                        {m.val}
                        <span className="diet-micro-unit">{m.unit}</span>
                      </span>
                    </div>
                    <div className="diet-micro-bar-wrap">
                      <div
                        className="diet-micro-bar"
                        style={{
                    ...
                    <div className="diet-micro-dv">
                      {Math.round((m.val / m.dv) * 100)}% of {m.dv}
                      {m.unit} daily value
                    </div>
                  </div>
                ))}
              ...
              <div className="diet-micros-disclaimer">
                ...
                These are <strong>approximate general values</strong> based on
                typical food composition (USDA data). Actual micronutrient
                content varies by brand, preparation method, and freshness.
                <span className="diet-micros-edit">
                  <RiEditLine size={11} /> Adjust manually if your foods differ.
                </span>
              </div>
            </>
          )}
